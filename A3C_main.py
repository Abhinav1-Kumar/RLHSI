...
if os.path.isdir('checkpoint'):
    try:
        checkpoint = torch.load(FLAGS.eval_net_path, map_location=torch.device('cpu'))
...
class ACNet(object):
    ...
    def _build_net(self, scope):
        ...
        with tf.variable_scope('actor'):
            l_a = tf.keras.layers.Dense(64, activation=tf.nn.relu6, 
                            kernel_initializer=w_init, name='la')(self.s)
#            l_a_norm = tf.contrib.layers.batch_norm(l_a, 0.9, epsilon=1e-5, activation_fn = None, is_training=True)
            l_a_2 = tf.keras.layers.Dense(64, activation=tf.nn.relu6, 
                              kernel_initializer=w_init, name='la_2')(l_a)
#            l_a_2_norm = tf.contrib.layers.batch_norm(l_a_2, 0.9, epsilon=1e-5, activation_fn = None, is_training=True)
            a_prob = tf.keras.layers.Dense(N_A, activation=tf.nn.softmax, 
                               kernel_initializer=w_init, name='ap')(l_a_2)

        with tf.variable_scope('critic'):
            l_c = tf.keras.layers.Dense(64, activation=tf.nn.relu6, 
                            kernel_initializer=w_init, name='lc')(self.s)
            l_c_2 = tf.keras.layers.Dense(32, activation=tf.nn.relu6, 
                              kernel_initializer=w_init, name='lc_2')(l_c)
            v = tf.keras.layers.Dense(1, kernel_initializer=w_init, name='v')(l_c_2)
